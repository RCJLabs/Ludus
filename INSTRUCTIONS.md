...
- Everything lives in `src/ludus.jsx` (~27,300 lines at v3.115.0). Edit it directly. `src/main.jsx` is a four-line mount point; don't grow it.
...
4. **Deliver the build.** Where `/mnt/user-data/outputs/` exists, copy `index.html`, `src/ludus.jsx` and `ROADMAP.md` there, re-zip the repo (exclude `node_modules`, `dist`, `package-lock.json`), and `present_files` with `index.html` first. **In the remote container that path does not exist** and delivery is the git push in step 5 — do not fake this step, say which one you did.

5. **Run `npm run test:all` and READ THE RED COUNT.** Not `npm test` — that silently skips nine browser checks (`modals, reach, room, sand, seller, surface, survive, sweep, words`) and says so in a line easy to scroll past. Only `test:all` is a release gate. Re-run it after any rebuild: a suite run against a tree you have since changed proves nothing about the tree you are shipping.
6. **Fast-forward `main` to the release.** The app builds from `main`, so work that stops at the feature branch is work Evan cannot see. `git push origin <branch>:main` once the suite is green. Four releases once sat on a branch while `main` served v3.79.0 and the app looked untouched — the pipeline is not finished until `main` carries the version you just bumped.

## Operational — learned the hard way
- **The container can reset the working tree to an old commit mid-session.** It happened three times in the v3.105–v3.115 session. Everything survived because it had been pushed. **Push before every long measurement.** The tells: a file you created is missing, `git log` shows a version you left behind, or `dist/test.html` changes size in a step that changed no source (it is 3,038KB at v3.115.0). A measurement taken against the wrong build looks completely plausible — the staleness guard catches *source newer than build*, not *both moved backwards together*.
- **Check a new check's name is free before writing the file.** Two existing checks were silently overwritten in one session; both times the suite stayed green and the only tell was a total that did not move. `bulk` now fails on a name/filename mismatch or a duplicate, but the cheap habit is `ls test/checks/<name>.mjs` first.
- **A probe's policy belongs in the rope, not in the probe.** A copy kept inside a probe is a second implementation to keep in step, and one of them bankrupted its own houses and had the game blamed for it.
