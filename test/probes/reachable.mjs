/* CAN EVERY BRANCH BE REACHED AT ALL? — the two templated beats of a bout, swept.
   ...
   `SALUTES`, `DEATHS` and their two pickers are exported on the test handle. This takes a REAL
   gladiator out of a real game state — so the object has every field the game gives it, not the
   eight a synthetic would have — clones it, and sweeps the fields the tables actually read across
   values chosen to straddle every threshold in them. For each sample it asks the picker which
...
const out = await p.evaluate(([N])=>{
  ...
  const tables = {
    salute: { T:A.SALUTES, pick:A.saluteLine },
    death:  { T:A.DEATHS,  pick:A.deathLine },
  };
  const rep = {};
  ...
  for(let i=0;i<N;i++){
    ...
    const foe = clone(FOE, { origin: sameOrigin ? man.origin : (man.origin === "Gaul" ? "Syrian" : "Gaul"),
      cls: sameCls ? man.cls : (man.cls === "Secutor" ? "Retiarius" : "Secutor") });
    const dpf = one(DPF);
    const c = { B:foe, oppName:foe.name, crowd:one(CROWD), stakes:one(STAKES),
      ctx:{ patron:one(PATRON) }, prA:pr, prB:pr,
      mobHis: dpf > 12, mobClear: Math.abs(dpf) > 12,
      round:one(ROUND), odds:one(ODDS) };

    for(const [kind, { T, pick }] of Object.entries(tables)){
      const r = rep[kind];
      let hitIdx = -1;
      for(let j=0;j<T.length;j++){
        let ok = false;
        try { ok = !!T[j].when(man, c); } catch(e){ ok = false; }
        if(ok){ r.sat[j]++; if(hitIdx < 0) hitIdx = j;
          if(!r.sample[j]){
            ...
            try { text = T[j].say(man, c); } catch(e){ threw = String(e && e.message || e); }
      ...
      let got = null;
      try { got = pick(man, c); } catch(e){ r.threw++; continue; }
      const why = dirty(got);
...
console.log(`  ${entries} authored branches across both tables`);
...
else console.log(`  Every entry of both tables was selected or shown satisfiable.\n`);
