/* THE YARD'S WEEK, AND THE DOING THAT LEADS A FACE

   (Named `yard`, not `board`: `board` is taken, by the check that holds the wager panel's quoted
   odds against what the book actually pays. This one is about the Doctore's Board, which is a
   different board entirely. I overwrote that file writing this one and only caught it because the
   suite reported 83 checks after I had supposedly added an 84th — the count was the tell, and it
   would have been a silent deletion of a whole check otherwise.)
