...
const ALLOWED = {
  ...
  doFight:        { max: 359, why: "Raised 357 -> 359 for #274 (v3.270.0): the singles bout passes the man to `cityAfter` so the town learns HIS name as well as the house's, and puts `hisTown` on the sim context for the missio box to read. Two lines, both handing an existing value to an existing call. a long sequential ledger, but its purse branch alone reads eleven enclosing locals — the seams are not narrow" },
