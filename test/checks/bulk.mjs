...
const ALLOWED = {
  ...
  EVENTS:         { max: 1093, why: "Raised 1092 -> 1093 for #243 phase 1 (v3.243.0): `bargain` gains ONE line — the merchant tie's yearly call is a branch on the card that already fires about twice a house, not a fifty-ninth event, and its four strings were lifted out to `KIN_BARGAIN` beside the wife's own block, which is exactly what the stash note below says this check asks for. Raised 1078 -> 1092 for the stash ask (#233, v3.185.0), on the same argument as roomFire: a designed question is the sanctioned way this table grows. Only `make` and a one-line `run` live here — the three answers and their arithmetic were lifted out to `stashAnswer` beside the machinery they use, which is what this check asked for the yard duel too. a table of fifty-eight events, not a function — length here is content, and the note on how the week's one question is drawn sits between the table and pickEvent, which is where this check attributes it. Raised 1080 -> 1110 for roomFire (#131's second loss, v3.30.0): a designed event is the sanctioned way this table grows, and the headroom is deliberately a few lines, not a hundred, so drift still says stop" },
