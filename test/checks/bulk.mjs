...
const ALLOWED = {
  ...
  doFight:        { max: 360, why: "Raised 359 -> 360 for #305 (v3.298.0): ONE line carrying `res.sigOdds` out to the fight modal. The crux quote is computed in `simulateFight`, where the real fighter is, because the face this function builds for the modal has no `tec` and the button was quoting 54 in 100 for every man in the game off `sigLand`'s default. Passing a finished number is the #230 division — the box shows what the roll used, it does not re-derive it. Raised 357 -> 359 for #274 (v3.270.0): the singles bout passes the man to `cityAfter` so the town learns HIS name as well as the house's, and puts `hisTown` on the sim context for the missio box to read. Two lines, both handing an existing value to an existing call. a long sequential ledger, but its purse branch alone reads eleven enclosing locals — the seams are not narrow" },
  FightModal:     { max: 399, why: "one React render; splitting it moves JSX around without making anything measurable" },
  EVENTS:         { max: 1098, why: "Raised 1095 -> 1098 for #270 (v3.266.0): the `freedman` handler now takes the player's answer. `run(d,ev)` had no `i` at all, so no freedman outcome could offer a choice even if it wanted to — #270's bench is the first that does. Three lines: a two-line note on why the answer was being thrown away, and one branch for the decline. The outcomes themselves live in FREEDMEN, not here. Raised 1094 -> 1095 for #243 phase 3 (v3.245.0): `herAsk` is ONE registry line on the same pattern as `wifeIll` below it — the mistress cannot be in `askPool`, which walks `activeG`, so her channel is `womanWeek` beside `askWeek` and its two conversations live in `HER_ASKS` with the rest of her. Raised 1093 -> 1094 for #243 phase 2 (v3.244.0): `wifeIll` is ONE registry line on `match`/`toga`/`daughter`'s own pattern — `make(){return null}` so the die never draws it, and a `run` that delegates to `resolveWifeIll` beside `familyWeek`, where the card and its three doors actually live. Raised 1092 -> 1093 for #243 phase 1 (v3.243.0): `bargain` gains ONE line — the merchant tie's yearly call is a branch on the card that already fires about twice a house, not a fifty-ninth event, and its four strings were lifted out to `KIN_BARGAIN` beside the wife's own block, which is exactly what the stash note below says this check asks for. Raised 1078 -> 1092 for the stash ask (#233, v3.185.0), on the same argument as roomFire: a designed question is the sanctioned way this table grows. Only `make` and a one-line `run` live here — the three answers and their arithmetic were lifted out to `stashAnswer` beside the machinery they use, which is what this check asked for the yard duel too. a table of fifty-eight events, not a function — length here is content, and the note on how the week's one question is drawn sits between the table and pickEvent, which is where this check attributes it. Raised 1080 -> 1110 for roomFire (#131's second loss, v3.30.0): a designed event is the sanctioned way this table grows, and the headroom is deliberately a few lines, not a hundred, so drift still says stop" },
  ...
  App:            { max: 5883, why: "Raised 5882 -> 5883 for #308 (v3.301.0): the morning report says why the weeks can or cannot be run on. ONE line of markup printing `runSays(S)`; the rule, the eleven terms and the measurement behind it all live at module scope over `runnable`, and the button itself got a line SHORTER by reading `runnable` instead of `weekWeight` plus `weeksToSomething`. #307 before it paid for its tag out of App's own dead code and raised nothing. Raised 5875 -> 5882 for #305 (v3.298.0): the review fixes. Four lines put the road's patron cost on the ON THE ROAD card as well as the in-town one — the weeks BETWEEN towns are charged the away rate too and the travel card returns before the city card ever renders, so half of `roadSaysFavour`'s own guard could never fire. Three more turn two double-invocations into the `(()=>{ const f = ...; })()` form the adjacent #298 and #302 blocks already use, which calls the function once instead of twice a render. Raised 5862 -> 5875 for #302 (v3.295.0): the booking modal gains WHAT IS AGAINST HIM — the same reading the result panel gives afterwards, run before the card is taken. Thirteen lines of one conditional block and a list. It adds NO rule and NO condition: `readBout` called without a `res` is pre-flight mode, so the fourteen rules that were always about the pairing rather than the outcome render from PRE_SAY instead of their own past tense, and the rest stand down. Proved byte-identical on the post-bout path over 3,000 swept states. Raised 5853 -> 5862 for #298/#300/#301 (v3.294.0): three lines the player reads before deciding, and none of them decides anything here. #298 adds five lines to the booking modal — what the sky is worth to the man you are about to send, against what it is worth to the other man; the figures come from `skySays`, which reads `skyMods`, the same call the bout makes with the same three arguments. #300 adds two lines at the crux — the ordered signature priced off `sigLand`, the one expression the round rolls against. #301 adds four lines to the in-town panel — what the road is costing in patron standing, built by `roadSaysFavour` off the two named constants the decay spends. Every one of the three is the division this allowance exists for: the number and its wording are built in the domain code and the panel prints them. Raised 5849 -> 5853 for #280 (v3.276.0): the in-town panel says what the road is costing the men RIGHT NOW, beside the button that ends it. Four lines — a two-line comment and one conditional div. The number and its wording are built in the domain code (`roadWear`, `roadSaysWear`); this only prints them, the same division `roadSays` set in #268 one panel above it, and the same division `pietySays` and `riteSays` set before that. It is deliberately NOT in the trade sentence above: `roadSays()` is the standing bargain a player weighs before he goes, and this is a running total that only exists once he has gone. Raised 5837 -> 5849 for #274 (v3.270.0): a man's sheet says where he is known on his OWN account, and only where that is not simply his renown — the item's own #101 note asks for the one that differs, not the table of three towns. Twelve lines of one conditional block. `manBestTown` and `manFollow` are in the domain code; this prints them. Raised 5814 -> 5837 for #269 (v3.265.0): the settings Guidance group gains HOW A HOUSE IS LOST — the nine ways a house is TAKEN, each with the engine's own figure for where this house stands against it. Twenty-three lines of one <details> and its rows, in the settings modal, which lives in App rather than SECT because it is not one of the thirty-two panels — the same place #265's recall list went. The list, what is on it and the argument for what is deliberately OFF it (`closed` and `triumph`, which are played toward, and the unreachable `oldAge`) are all in the domain code over `END_DOORS`; this only prints them. Raised 5810 -> 5814 for #268 (v3.264.0): the two road panels each gain the line saying what the road costs — a comment and one `roadSays()` div apiece, in the circuit panel where a tour is chosen and in the in-town panel beside the purse multiplier it trades against. Both figures are counted off `EV_HOME` and `NIGHT_KEYS` in the domain code and the panels only print them, the same division `riteSays` and `pietySays` set in the two releases before this. Raised 5786 -> 5810 for #265 (v3.261.0): the Guidance group gains the gatekeeper's recall list — what he has already said, each title re-openable over its text. Twenty-four lines of list and an empty state, in the settings modal, which lives in App rather than in SECT because it is not one of the thirty-two panels. The list itself is built in the domain code (`lessonsTold`) and this only prints it, the same division `RITE_TERM` set one release earlier. the whole screen minus its panels — tabs, faces, state and handlers, and nothing else: the test handle after it is no longer charged here. Was 7,200 when the thirty-two <Sect> panels were inline; they are in SECT now and this came down with them" },
  SECT:           { max: 1517, why: "Raised 1497 -> 1517 for #299 (v3.294.0): the standing panel gains WHAT CAPUA REMEMBERS OF YOU — the six legacies, how close the next one is, and which of them is still working. They carry real weight (a senator already yours, every price 6% cheaper for ever, every man +6 regard) and rendered in exactly ONE place before this: the title screen, between runs. Twenty lines of rows and an empty state. Every word of it — the names, the units, what a held legacy is worth and whether it is a standing effect or a gift already spent — is built in `legacyRows` and `LEGACY_WORTH` in the domain code, which is what this allowance is for; the panel decides nothing. Raised 1494 -> 1497 for #268 (v3.264.0): the cells section names the one asymmetry a player meets on the road — the feast travels and the walk does not. Three lines, a conditional div and its text; NEITHER GATE MOVES, and the rationale for the line lives beside `roadSays` in the domain code rather than here, which is what this allowance is for. Raised 1492 -> 1494 for #267 (v3.263.0): the temple panel names what the band of piety is worth. Two lines — a one-line comment and the one `pietySays(S)` div. The terms and the omen figure are built in the domain code (`PIETY_TIERS`, `PIETY_TERM`, `omenIll`, `pietySays`) and the panel only prints them, the same division `riteSays` set one release earlier; the same release DELETED the panel's two bare copies of the godless threshold, so the markup is two lines longer and two thresholds shorter. Raised 1489 -> 1492 for #264 (v3.260.0): the rite panel names its terms. Three lines — a two-line comment pointing at the entrance panel this borrows its idiom from, and the one `riteSays(k)` div. The numbers themselves are built in the domain code (`RITE_TERM`, `riteSays`) and the panel only prints them, which is the same division `wifeWord` set below. Raised 1485 -> 1489 for #263 (v3.257.0): the party panel's button gains a REASON. `hostParty` refuses to throw a party from down the bay now — the patrons' favour decays at 2.5x while the house is away and `askWant` stops asking, and this function was raising every Capua patron's standing from two hundred miles off — so the button says which town you are standing in instead of looking live and doing nothing. Four lines: a comment, and the two branches of the label. Raised 1484 -> 1485 for #243 phase 2 (v3.244.0): the blood-of-the-house panel gains ONE row for a widower — who he is missing, and whether her people still count his children as theirs. `wifeWord` builds it in the domain code; the panel prints it. Raised 1483 -> 1484 for #243 phase 1 (v3.243.0): the blood-of-the-house panel gains ONE row, the line that says what her family is worth — `wifeWord` builds it out in the domain code and the panel only prints it. a registry of thirty-two panels, not a function — length here is content, the same argument EVENTS makes. Each entry is one <Sect> lifted out of App verbatim, so the total is the markup that was already there; what changed is that a face is now a readable list of what it shows, in order, which is the whole point of the overhaul. It grows when the game gains a panel, and the headroom is a hundred lines rather than a thousand so drift still says stop" },
  ...
  CSS:            { max: 274, why: "the stylesheet, plus the four @font-face rules the embedded faces need and the eight venue backdrops. RAISED 264 -> 274 IN v3.152.0 FOR THE PLATE — ten lines, one component: the room\'s own picture at the top of its page, sticky under the header and folding to a band on scroll. It is the only fixed furniture in a room and it replaces nothing, so the file grows by what it is worth. It could not be split the way App was in the same release: App shed its new lines into `CircuitLedger`, but a stylesheet has nowhere to shed to. It was 279 first — the raise was written at 275 and then the plate\'s explanation was written BESIDE THE RULES, which is four lines of prose measured by this sweep and shipped to the browser. It sits above `const CSS` now, where the @font-face note below says such things go, and the twelve lines of it cost nothing here. Raised 253 -> 264 in v3.138.0 for Vmbra, and the reason is structural rather than drift: a shadow play needs a LIT SURFACE for a shadow to be a shadow on, and `.umbra` is that surface — one rule, used anywhere a figure is drawn outside the arena. It REPLACED an inline gradient written at the man's-page call site, so the file did not grow by what the stylesheet grew by. The other lines are the three notes on why every venue backdrop inverted and why the colourblind pref now moves blood instead of green, both of which are exactly the kind of thing that gets undone by somebody who does not know it was deliberate" },
};
