...
| `patron` | fast | #131's first piece, and the first thing a great house can lose. A year-12 house holds four patrons at a mean favour of 94 of 100 and **nothing in the file ever took one away** — favour decayed, the man stayed, which is the shape of a late game where 97.7% of what is shown was available in week one. They die now, and `patronWeek`'s existing refill reseats the rank with a stranger at `ri(28,42)`. **The cost does not land where the first version of this check asserted it**: `recomputeFavor` is a weighted mean, so removing one of two men both at 90 leaves standing at 90 — measured 90 → 90 on the day → **54 once the seat refilled**. The bar moved to the house a few weeks later, which is also what a player experiences. Holds four things and the first would ruin a save: never the last patron (`recomputeFavor` sets `d.favor = 0` on an empty list, so a house that could lose its last man spirals instead of suffering); the man actually leaves and standing falls; the gate needs 60 weeks of service and favour 55, so a young house cannot meet it however unlucky and a long-held man the town thinks little of is not the one who dies; and mourning buys what the second door promises — the next patron arrives at 52 rather than 34, inside a 26-week window |
