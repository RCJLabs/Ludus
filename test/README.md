...
Nine drive a real browser through the real screens, and those cost minutes.
...
| `actions` | fast | fifty-eight more actions lived in that same closure, and one probe measured its own copy of the feast. **From v3.23.0 the list is DERIVED as well as declared**, because a hand-written list catches an action that goes missing and is silent about one nobody ever thought to list — a blind spot that has cost five findings: `setOut`/`comeHome` (two 12-house batches emigrated and reported half the game dark), `nameHeir` (the heir null in 8 of 8), `makeMarket` (a block battery measuring the founding stall five times), `holdMunera` (`d.honoured` reading 0 in every measurement ever taken here), and the nineteen this sweep found. `coverage` cannot see it either — it reports what no check TOUCHES, among things already on the handle. The definition is not "mutates the save", which sweeps in every internal helper, but "the UI calls it inside a `mut(d => …)` closure", which IS the player doing something; the source is read directly, like `layers` and half of `saves`, and comments are stripped first because the v3.23.0 note explaining this sweep contained the words `mut(d => …)` and the sweep matched its own documentation |
...
| `card` | fast | three of the four fight engines shared a quarter of the bill, and the audit that found it was counting Rome. **From v3.26.0 its size bar is re-baselined and its sample tripled**, because the bar had been fitted to the small one: 3 houses gave 38–53 cards and bouts-per-card swung 2.71 to 3.36 across RNG trajectories with no game change — proved by adding one key to `EVENTS` whose `make` returns null, which reshuffles `pickEvent` and every draw after it. At 10 houses the figure is 2.90–2.94, always below the old 3.2 bound. #127's rule applied: measure the distribution, do not nudge the threshold |
| `stone` | fast | 336,500 denarii of works and monuments, and the richest careful house ever measured held 8,485. **From v3.31.0 it holds what a work's `say` CLAIMS against what its perk does, and from v3.32.0 that every perk is DELIVERED at the size its own table names** — driven through the game's own `worksWeek`, because a perk silently disconnected reads exactly like a perk that does not matter, and telling those two apart is what #140 and #141 spent two releases getting wrong (verified by disconnecting the chapel's calm line: it fails, naming the figure). (#140): the tomb's line read "a death costs the cells far less", and its second effect lands on the LANISTA — `workPerk(d,"regard")` multiplies his health loss by 0.7 at all four death sites, while the cells' share of a death is softened by `collSoften`, the burial society. One purchase's effect credited to another's, which is the `near`/`words` class. Pinned by MECHANISM rather than copy: it calls `workPerk` (put on the handle so a check need not reconstruct the formula) and reads the source for where the 0.7 sits, failing if those sites stop being the lanista's health or if the wording goes back to naming the cells. Verified against the old line — it fails, naming the fault |
...
| `policy` | fast | the reference player, and the bar under him. `survive` asks whether a NEW house can get off the ground; this asks whether one played WELL still gets anywhere — **24 houses × 320 weeks from v3.30.0, was 8** — through `__ROPE.play`, in about 8 seconds. The bars are on the BEST house in the run, not the median: measured over 48 houses, 38% die before week 100 and 29% are still standing at the wall, so the median of eight spans 54 to 321 weeks and would fail a bar of 70 in one run of three with no change to the game. Best-of-run life spans 291 to 321 and fame 6,808 to 17,487; the bars are 150 and 2,000. Widened because the roomFire key's reshuffle caught two more bars fitted to one trajectory (#136's shape, third instance): at n=8, best-rooms swung 1..4 across seven dead-key trajectories against a bar of 3 and the census floor tripped once — two false failures in seven runs with no game change. At 24 the same sweep reads rooms 4-5 and the floor holds with real margin |
...
| `quiet` | fast | the only multi-week advance in the game was an early-game-only button by accident. `weekWeight` calls a week `quiet` only at load 0, and that is the sole gate on "Let it run · Nw" — but one of its load terms read the unburied dead with NO window, where every other reader of that list has one (the agenda line says "after this nobody can put it right"; the villa section is captioned "6w to decide"). `markUnburied` keeps the last fourteen men for ever and only `holdMunera` clears one, which the UI offers inside the window alone — so past six weeks the entry was permanent, the player had no action left that could clear it, and load could never return to 0. Measured over 10 houses × 420 weeks: the term fired on **95.6% of all house-weeks**, all ten houses were stuck for good between week 10 and week 37, and **every house's last quiet week fell inside its first 13**; windowed, it moves to week 145–325 for seven of them and quiet weeks go 1.8% → 4.1% of play. On one seed, 3 quiet weeks ending week 6 became 9 ending week 172. It was invisible because `weekWeight`, `unhonoured` and `holdMunera` were all off the handle — `holdMunera`'s only caller was the UI closure at 18649 — so no check could ask the week its shape or bury a man, and `d.honoured` read 0 in every measurement ever taken here. This holds the rule rather than the number, and drives the three rites end to end since they had never once been driven |
| `patron` | fast | #131's first piece, and the first thing a great house can lose. A year-12 house holds four patrons at a mean favour of 94 of 100 and **nothing in the file ever took one away** — favour decayed, the man stayed, which is the shape of a late game where 97.7% of what is shown was available in week one. They die now, and `patronWeek`'s existing refill reseats the rank with a stranger. **Three things it knows that the first draft did not, each caught by measuring:** the cost lands on the REPLACEMENT, not the death, because `recomputeFavor` is a weighted mean — 90 → 90 on the day → 55 once the seat refilled, and the first version asserted the drop on the day and failed correctly; the gate must be late **by the lanista's own clock**, since `PATRON_HELD`'s 60 weeks is year FOUR and deaths were landing on houses that then died at 70-86 weeks, which `policy` read as the late game going out of reach (fame 1,645 against a control at 3,720, five subsystems dark); and never the last patron, since `recomputeFavor` sets `d.favor = 0` on an empty list and a house that could lose its last man would spiral rather than suffer. Also holds that mourning buys what its door promises, and that the 120-week gap shuts it straight after — without which it takes one man after another until the house is all strangers |
| `blaze` | fast | #131's second loss, and the second thing a great house can lose: measured over 288 house-runs, no house ever lost a room. The fire takes the TOP LEVEL of one built-up wing — never the wing, so nothing reading presence (staff rooms, charter, lessons) loses its footing — and rebuilds through `buildUp` at the level's own price. Holds the patron piece's three lessons as rules: late by the ESTATE's clock (8 built levels, so a young house cannot be touched — the first patron draft gutted houses at week 60-86), a 100-week gap so fires do not chain, and a loss rather than a wound (the floor goes whichever door is taken; the choice trades the men's bodies against the stores). Its release also caught the THIRD instance of #136's shape: the new key's reshuffle false-failed `policy` and `steel` on bars fitted to one trajectory, proved by the failures reproducing identically with the event's `make` neutered to `return null` |
| `named` | fast | the fighter-nemesis had one designed payoff — beat the name on the sand — and it fired on 1-2% of episodes, because a pit man was named nemesis under a synthetic small-house name that `nemesisWeek`'s rivals-only lookup could never resolve, so 99-100% of circuit-born nemeses were unmade THE SAME WEEK they were named and 82-87% of rival episodes died by eviction under a usurper who then evaporated. Invisible to every weekly instrument: the clear rate was three times the visible episode rate, so most episodes lived and died between two week-end reads — `ghost.mjs`'s setter trap is what caught it. Holds the rules, not the trajectory: cleared only when genuinely gone from every roster and the circuit, a killer takes the title from a non-hated name and not from a hated one (the fix's own second fault — the title chained hands every 4.4 weeks until that clause), the quiet exit writes its line, and a pit win brings the name down and lifts the yard |
| `steel` | fast | **its armoury comparison was ONE house against one with a strict inequality until v3.27.0 (#136)**, so it flipped on a pure RNG reshuffle; pooled over 5 houses an arm and ~5,000 man-slot-weeks the effect is unmistakable — 88.2% keen against 79.8%, worn-or-worse 2.5% against 8.1%. Wear — the one system where the probe was wrong FOUR separate ways. #114 read `d.gearCond`, which is the pool of pieces on the SHELF, and concluded steel never wears; read off the man in `g.wear[slot]` a bout takes 3-6 off a weapon, all five words are said and pieces break. Holds the rate against `WEAR_RATE`, the five words off a piece driven to nothing, the break on the game's own chronicle line, the bands a played house sees, a man's career against his weapon's life, and — the trap that cost the most — that a bout held at the balance has changed nothing while the same bout answered changes the kit |
